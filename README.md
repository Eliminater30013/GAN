...
Code, dataset, and training models for a Generative Adversarial Network (GAN) produced during the Nottingham Summer Engineering Research Programme and into the first half of my second year of university. The GAN is able to detect the changes in optical properties and by utilising Spatial Frequency Domain Imaging (SFDI), it can detect malformed tissue structures in rectangular and cylindrical geometries for simulated tissue. Adapted from Mason Chen's GANPOP [model](https://github.com/masontchen/GANPOP_Pytorch). A poster summarising my research can be found [here](https://github.com/Eliminater30013/GAN/blob/main/research/Research%20Poster.pdf).
...
The complex rectangular dataset was generated using '*Rectangular.Blend*', '*RectangularCurved.blend*', '*RectangularRagged.blend*' and '*RectangularCurvedTumour*' and the cylindrical dataset was generated using '*Cyl_tumours.blend*'. The optical properties of the image were changed by varying final fac (the proportion between scattering and absorption), abs fact (the absorption factor) and the sct (the scattering factor). Ground truths were generated by varying the red and green channels whilst keeping the blue channel of the image at 0. Note that abs was correlated to red and sct was correlated to green. Blender Models were created By Jane Crowley, a link to her paper can be found [here](https://doi.org/10.1117/12.2576779)
